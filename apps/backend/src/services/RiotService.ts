...
export class RiotService {
  static async getAccountByRiotId(riotId: string, tagline: string) {
    ...
    try {
      const response = await axios.get(
        `https://europe.api.riotgames.com/riot/account/v1/accounts/by-riot-id/${encodeURIComponent(riotId)}/${encodeURIComponent(tagline)}`,
  ...
  static async getSummonerByPuuid(puuid: string) {
    ...
    try {
      const response = await axios.get(
        `https://euw1.api.riotgames.com/lol/summoner/v4/summoners/by-puuid/${encodeURIComponent(puuid)}`,
        { headers: { "X-Riot-Token": process.env.RIOT_API_KEY } }
      );
      return response.data;
    } catch (error: any) {
  ...
  static async getListMatchesByPuuid(puuid: string) {
    try {
      const response = await axios.get(
        `https://europe.api.riotgames.com/lol/match/v5/matches/by-puuid/${encodeURIComponent(puuid)}/ids?start=0&count=1`,
        { headers: { "X-Riot-Token": process.env.RIOT_API_KEY } }
      );
      return response.data;
    } catch (error: any) {
  ...
  static async getLeagueEntries(summonerId: string) {
    ...
    try {
      const response = await axios.get(
        `https://euw1.api.riotgames.com/lol/league/v4/entries/by-summoner/${encodeURIComponent(summonerId)}`,
        { headers: { "X-Riot-Token": process.env.RIOT_API_KEY } }
      );
      return response.data;
    } catch (error: any) {
